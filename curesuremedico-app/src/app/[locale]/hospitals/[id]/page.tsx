...
export default function HospitalProfilePage() {
  ...
  if (loading) {
    return <main className="pt-20 min-h-screen flex items-center justify-center bg-surface">Loading hospital details...</main>;
  }
          ...
          <div className="lg:col-span-7">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div className="flex items-center gap-1 text-secondary font-bold">
                ...
                <span className="text-sm">{hospital.rating || "4.9"} Rating</span>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 bg-primary-container rounded-xl p-8 text-on-primary-container flex flex-col justify-between">
              ...
              <div>
                <div className="text-4xl font-black mb-2">{hospital.established_year ? new Date().getFullYear() - hospital.established_year + "+" : "Premium"}</div>
                <div className="text-sm font-medium opacity-80 uppercase tracking-widest">{hospital.established_year ? "Years of Legacy" : "Care"}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            ...
            <div className="flex gap-4">
              <div className="text-right">
                <span className="block text-3xl font-black text-secondary">{hospital.beds_count || "200+"}</span>
                <span className="text-xs text-on-surface-variant font-bold uppercase tracking-tighter">Premium Beds</span>
              </div>
              <div className="w-px h-10 bg-outline-variant/30"></div>
              <div className="text-right">
                <span className="block text-3xl font-black text-secondary">{hospital.ots_count || "5+"}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {hospital.facilities && hospital.facilities.length > 0 ? (
              hospital.facilities.map((fac: any, index: number) => {
                const colors = ["primary", "secondary", "tertiary"];
                const color = colors[index % colors.length];
                return (
                  <div key={index} className={`bg-surface-container-lowest p-8 rounded-xl border-b-4 border-${color}`}>
                    <span className={`material-symbols-outlined text-${color} text-4xl mb-6`}>{fac.icon || "medical_services"}</span>
                    <h4 className="text-xl font-bold mb-3">{fac.title}</h4>
                    <p className="text-on-surface-variant text-sm leading-relaxed">{fac.description}</p>
                  </div>
                );
              })
            ) : (
              <div className="col-span-3 text-center text-on-surface-variant">Facilities information not available.</div>
            )}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {specialtiesList.length > 0 ? (
              specialtiesList.map((spec: string, index: number) => {
                const icons = ["cardiology", "medication_liquid", "orthopedics", "smart_toy", "neurology", "pulmonology", "gastroenterology"];
              ...
              <div className="col-span-4 text-center text-on-surface-variant">Specialties information not available.</div>
            )}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 h-[600px]">
            <div className="md:col-span-6 rounded-xl overflow-hidden relative">
              <img className="w-full h-full object-cover" src={hospital.gallery_image_1 || hospital.image_url} alt="Gallery Image 1" />
            </div>
            <div className="md:col-span-3 grid grid-rows-2 gap-4">
              <div className="rounded-xl overflow-hidden relative">
                <img className="w-full h-full object-cover" src={hospital.gallery_image_2 || hospital.image_url} alt="Gallery Image 2" />
              </div>
              <div className="rounded-xl overflow-hidden relative">
                <img className="w-full h-full object-cover" src={hospital.gallery_image_3 || hospital.image_url} alt="Gallery Image 3" />
              </div>
            </div>
            <div className="md:col-span-3 rounded-xl overflow-hidden relative">
              <img className="w-full h-full object-cover" src={hospital.gallery_image_4 || hospital.image_url} alt="Gallery Image 4" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative mt-8 lg:mt-0">
            ...
            <div className="md:absolute -bottom-6 -left-6 bg-surface-container-lowest p-6 rounded-2xl shadow-lg border border-outline-variant/10 max-w-[240px]">
              ...
              <p className="text-xl font-black">{hospital.contact_number || "+91 00 0000 0000"}</p>
