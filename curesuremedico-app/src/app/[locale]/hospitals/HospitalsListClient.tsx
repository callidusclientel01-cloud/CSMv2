...
export default function HospitalsListClient({ initialHospitals }: Props) {
  ...
  }, [initialHospitals, searchParams, selectedCities, selectedSpecialty, selectedCountries, locale]);
  ...
  const availableCities = Array.from(new Set(initialHospitals.map(h => {
    if (!h.city) return null;
    const parts = h.city.split(',');
    return parts[0].trim();
  }))).filter(Boolean).sort() as string[];
  
  const availableCountries = Array.from(new Set(initialHospitals.map(h => h.country))).filter(Boolean).sort() as string[];
  const availableSpecialties = Array.from(new Set(initialHospitals.flatMap(h => h.specialties || []))).filter(Boolean).sort();
                  ...
                  <div className="flex-1 p-8 flex flex-col">
                    ...
                    <div className="mt-auto flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 pt-6 border-t border-outline-variant/20">
                      ...
                      <div className="flex flex-wrap sm:flex-nowrap gap-4 w-full xl:w-auto mt-2 xl:mt-0 items-center justify-between xl:justify-end">
                        <Link href={`/${locale}/hospitals/${hospital.slug || hospital.id}`} className="text-sm flex items-center gap-1 font-bold text-primary hover:underline whitespace-nowrap">
                          {t("viewDetails")} <span className="material-symbols-outlined text-sm">arrow_forward</span>
                        </Link>
                        <Link href={`/${locale}/quote`} className="bg-primary text-center w-full sm:w-auto text-on-primary px-6 py-2 rounded-full text-sm font-bold shadow-sm hover:opacity-90 whitespace-nowrap">
                          {t("bookConsultation")}
                        </Link>
                      </div>
                    </div>
