  ...
  const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          // Add a loading toast
          toast.loading("Importing hospitals...", { id: "import" });
          
          // results.data is an array of objects representing rows
          const rowsToInsert = results.data.map((row: any) => ({
            ...row,
            // Provide defaults if certain fields are empty
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg overflow-hidden bg-slate-200 shrink-0">
                          {hospital.image_url ? (
                            ...
                            <span className="material-symbols-outlined w-full h-full flex items-center justify-center text-slate-400 text-sm">local_hospital</span>
                          )}
                        </div>
