document.addEventListener('DOMContentLoaded', () => {
    const countries = [
        ...
        { name: "Congo, Republic of the", code: "+242" }, { name: "Costa Rica", code: "+506" }, { name: "Cote d'Ivoire", code: "+225" },
        ...
        { name: "Korea, South", code: "+82" }, { name: "Kuwait", code: "+965" }, { name: "Kyrgyzstan", code: "+996" },
    ...
    countrySelects.forEach(select => {
        // Simple heuristic: if the first few options include Nigeria, Kenya, etc.
        const optionsText = Array.from(select.options).map(o => o.text.toLowerCase());
        if (optionsText.includes('nigeria') || optionsText.includes('kenya') || select.closest('label')?.textContent.toLowerCase().includes('country')) {
            // It's a country select
            ...
            // Find relevant phone field
            const form = select.closest('form');
            if (form) {
                const phoneInput = form.querySelector('input[type="tel"]');
                let prefixSpan = form.querySelector('.flex.items-center span.font-bold');
                ...
                select.addEventListener('change', (e) => {
                    ...
                    if (phoneInput) {
                        // Keep user's number but swap prefix
                        let currentVal = phoneInput.value.trim();
                        // if value starts with a +, we replace the +... up to first space, else just set it
                        if (currentVal.startsWith('+')) {
                            const withoutPrefix = currentVal.replace(/^\+\d+/, '').trim();
                            phoneInput.value = code + ' ' + withoutPrefix;
                        } else {
